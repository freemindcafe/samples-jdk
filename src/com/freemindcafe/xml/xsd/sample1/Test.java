...
import com.freemindcafe.purchase.PurchaseOrderDocument;

public class Test {
	
	@org.junit.Test
	public void read_xml_and_validate_against_the_schema_fails() throws Exception{
		InputStream stream = this.getClass().getResourceAsStream("/com/freemindcafe/xml/xsd/sample1/po.xml");
	...
	public void read_xml_and_validate_against_the_schema_succeeds() throws Exception{
		InputStream stream = this.getClass().getResourceAsStream("/com/freemindcafe/xml/xsd/sample1/po-default.xml");
		PurchaseOrderDocument document = PurchaseOrderDocument.Factory.parse(stream);
		XmlOptions options = new XmlOptions();
		ArrayList<Object> validationErrors=new ArrayList<Object>();
		options.setErrorListener(validationErrors);
